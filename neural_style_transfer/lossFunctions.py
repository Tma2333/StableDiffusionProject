...
class ContentLoss(nn.Module):
    ...
    def forward(self, inputs):
        ...
        self.loss = self.weight*F.mse_loss(inputs[:length], self.target[:length])
        return inputs
    ...
    def forward(self, inputs):
        ...
        for i in range(len(Gs)):
            for j in range(len(self.Gs_orig)):
                # if i ==j:
                #     continue
                self.loss = torch.add(self.loss, F.mse_loss(Gs[i], self.Gs_orig[0]))
        self.loss = self.weight*self.loss#.div(len(Gs)**2/2)
        return inputs
